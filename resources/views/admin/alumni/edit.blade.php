                ...
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('admin.alumni.update', $user) }}" enctype="multipart/form-data">
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="phone_number" class="block mt-1 w-full" type="text" name="phone_number" :value="old('phone_number', $user->alumniProfile->phone_number ?? '')" />
                            <x-input-error :messages="$errors->get('phone_number')" class="mt-2" />
                        </div>

                        <div class="mt-4">
                            <x-input-label for="bio" :value="__('Bio')" />
                            <textarea id="bio" name="bio" class="border-gray-300 focus:border-tukutane-red focus:ring-tukutane-red rounded-md shadow-sm mt-1 block w-full" rows="5">{{ old('bio', $user->alumniProfile->bio ?? '') }}</textarea>
                            <x-input-error :messages="$errors->get('bio')" class="mt-2" />
                        </div>

                        <div class="mt-4">
                            <x-input-label for="linkedin_url" :value="__('LinkedIn URL')" />
                            <x-text-input id="linkedin_url" class="block mt-1 w-full" type="url" name="linkedin_url" :value="old('linkedin_url', $user->alumniProfile->linkedin_url ?? '')" />
                            <x-input-error :messages="$errors->get('linkedin_url')" class="mt-2" />
                        </div>

                        <div class="mt-4">
                            <x-input-label for="github_url" :value="__('GitHub URL')" />
                            <x-text-input id="github_url" class="block mt-1 w-full" type="url" name="github_url" :value="old('github_url', $user->alumniProfile->github_url ?? '')" />
                            <x-input-error :messages="$errors->get('github_url')" class="mt-2" />
                        </div>

                        <div class="mt-4">
                            <x-input-label for="current_company" :value="__('Current Company')" />
                            <x-text-input id="current_company" class="block mt-1 w-full" type="text" name="current_company" :value="old('current_company', $user->alumniProfile->current_company ?? '')" />
                            <x-input-error :messages="$errors->get('current_company')" class="mt-2" />
                        </div>

                        <div class="mt-4">
                            <x-input-label for="job_title" :value="__('Job Title')" />
                            <x-text-input id="job_title" class="block mt-1 w-full" type="text" name="job_title" :value="old('job_title', $user->alumniProfile->job_title ?? '')" />
